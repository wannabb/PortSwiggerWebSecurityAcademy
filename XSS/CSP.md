...
- 개념: CSP의 주요 목표는 XSS 공격을 완화하고 보고하는 것입니다. XSS 공격은 서버에서 받은 콘텐츠를 브라우저가 신뢰한다는 점을 악용합니다.
  브라우저는 콘텐츠의 출처를 신뢰하기 때문에 콘텐츠가 이상한 곳에서 오더라도 악성 스크립트를 피해자의 브라우저에서 실행합니다.
...
- 일부 정책은 매우 엄격하여 외부요청을 모든 형태의 외부요청을 막지만, 사용자 상호작용을 유도하여 이러한 제한을 우회하는 방법도 존재한다. [엄격한 CSP 우회](https://not)
